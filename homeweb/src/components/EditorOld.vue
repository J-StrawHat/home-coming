<template>
  <div>
    ...
    <el-form :model="addList" :rules="rules" ref="Form" label-width="100px" class="demo-ruleForm" >
      <el-row >
        ...
        <el-col :span="4" style="margin-left: 20px" >
          <el-upload style="border: 1px dashed white;width: 120px"
            ...
            <img v-if="addList.photo" :src="addList.photo" class="avatar">
      ...
      <el-form-item>
        <el-button type="primary" @click="updateold">更新</el-button>
      </el-form-item>
...
export default {
  name: "EditorOld",
  mounted() {
    getRequest("/getELderById/"+this.$route.query.id).then(
        (result)=>{
          this.addList=result;

          this.changeProvince(this.addList.missProvince)
          this.changeCity(this.addList.missProvince,this.addList.missCity)
          this.changeCounty(this.addList.missProvince,this.addList.city,this.addList.missCounty)
        }
    )
    this.list=d;
    for (let i in d) {
  ...
  methods: {
    ...
    updateold()
    {
      putRequest("/Elder",this.addList).then(
          (result)=>
          {
